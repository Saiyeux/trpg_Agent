...
- 如果没有明确目标，使用"周围"、"环境"等通用描述

## 输出格式
请返回纯JSON格式，不要使用代码块包装：
{{
    "type": "执行",
    "category": "攻击|搜索|对话|交易|移动|状态查询|其他",
    "target": "明确的目标对象（如哥布林、周围、商人等）",
    "description": "玩家想要执行的具体动作描述"
}}

## 严格要求
1. category必须精确使用以下值之一：攻击|搜索|对话|交易|移动|状态查询|其他
2. 禁止在category中添加"类"字，如"对话类"是错误的，应该是"对话"
3. target必须具体明确，避免使用"某个"、"一些"等模糊词汇
