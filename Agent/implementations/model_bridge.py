...
**状态查询**: 包含"状态"、"属性"、"背包"、"血量"、"查看我的"等词汇
**交互**: 包含"撬锁"、"开锁"、"激活"、"触发"、"操作"、"拉"、"推"、"按"、"转动"等词汇
**技能**: 包含"施放"、"施展"、"释放"、"使用技能"、"魔法"、"咒语"、"法术"、"治疗"、"加持"、"诅咒"等词汇
...
- **【绝对禁止】将用户提到的角色名替换为预设NPC名或其他名称**
- **【检查】输出前务必检查：用户是否明确提到了目标？没有则必须标记"未指定"**

## 【特别警告】目标映射错误纠正
❌ 严重错误: "酒馆老板" → target: "商人老约翰" (绝对禁止名称替换!)
✅ 正确示例: "酒馆老板" → target: "酒馆老板"
❌ 严重错误: "村长" → target: "商人老约翰" (绝对禁止名称替换!)
✅ 正确示例: "村长" → target: "村长"
❌ 错误示例: "攻击" → target: "哥布林" (用户没提到哥布林!)
✅ 正确示例: "攻击" → target: "未指定"
❌ 错误示例: "施放火球术" → target: "哥布林" (用户没提到哥布林!)
✅ 正确示例: "施放火球术" → category: "技能", target: "未指定"
❌ 错误示例: "使用技能" → category: "其他" (技能使用有专门分类!)
✅ 正确示例: "使用技能" → category: "技能", target: "未指定"

## 输出格式
请返回纯JSON格式，不要使用代码块包装：
{{
    "type": "执行",
    "category": "攻击|搜索|对话|交易|移动|状态查询|交互|技能|其他",
    "target": "明确的目标对象（如哥布林、商人、宝箱）或'未指定'",
    "description": "玩家想要执行的具体动作描述"
}}

## 严格要求
1. category必须精确使用以下值之一：攻击|搜索|对话|交易|移动|状态查询|交互|技能|其他
...
- "和酒馆老板对话" → category: "对话", target: "酒馆老板"
- "向酒馆老板购买物品" → category: "交易", target: "酒馆老板"
...
- "施放火球术" → category: "技能", target: "未指定"
- "治疗术" → category: "技能", target: "未指定"
